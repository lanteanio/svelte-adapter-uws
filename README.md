...
| `relay_spill_pending_age_seconds`                         | gauge     | max            | Worst oldest-pending relay spill age observed at quarantine; `0` until a worker has seen one, so healthy is queryable as zero rather than absent. Compare with `CLUSTER_RELAY_MAX_PENDING_MS`; no topic or client identity is exposed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `CLUSTER_RELAY_MAX_FRAME_KB`  | `CLUSTER_RELAY_MAX_PENDING_KB` | Largest serialized envelope a worker will hand to the cluster relay (KB). This is the SENDER's ceiling and answers a different question from the two above: those describe a receiving worker that has stopped draining, this describes one frame that is too large to carry, which is not any worker's fault. It is decided once per publish, so every worker gets the same answer and none is left silently one frame behind; a refused publish still reaches the publishing worker's own subscribers, and is reported as `cluster-relay.frame-refused`. Applies whether or not the rings are enabled. `0` disables it |
...
In a runtime with more than one worker, the implicit counter is refused: separate workers cannot allocate one monotonic topic sequence. Choose `{ seq: false }` for volatile data, or use `{ seq: authoritativeNumber, relay: false }` after an external ordered source (for example one Redis Stream consumer group or database outbox sequencer) has allocated the number and will fan the event to every process. A numeric seq without `relay:false` is also refused: allocation order alone does not serialize the built-in multi-origin relay. The same rule covers `publishWire` and every entry in `publishBatched`. `publishWireBatch` is stricter and simpler: it carries one options object with no per-entry sequence, so a numeric `seq` is refused there outright - on every topology, and whether the call is publishing twenty entries, one, or none. Publish through `publishWire` when each frame needs its own authoritative number.
