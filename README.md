...
| `psi_cpu_some_avg10` | gauge | max | Kernel pressure-stall CPU `some` avg10: percent of the last 10s any task was stalled on CPU. Registered only where the kernel exposes PSI, probed once at startup, so absent elsewhere rather than reporting a zero that reads as "no pressure". |
| `psi_memory_full_avg10` | gauge | max | Kernel pressure-stall memory `full` avg10. Same availability note. |
| `psi_io_full_avg10` | gauge | max | Kernel pressure-stall IO `full` avg10. Same availability note. |
| `cpu_throttled_ratio` | gauge | max | Fraction of the sampled window the cgroup CPU quota held this process suspended. Registered only where a cgroup CPU stat is readable, probed once at startup. |
...
| `metrics_snapshot_workers_reporting` | gauge | max | Workers that answered before the deadline. Below `_expected` means the merged document is partial - every summed series is understated, and a dip must not be read as a real drop in traffic. |
...
Each posture change also logs one `[ws] protection posture <from> -> <to>` line with the rolling reject rate and the base pressure reason at the moment of transition. Costs when enabled: one unlabelled counter increment per accepted upgrade, one labelled increment per rejection, and a fixed set of gauge writes per pressure sample (roughly a dozen and a half, all of them scalars the sampler had already computed for its own use); when off, every site is a single undefined check. Nothing here runs on a per-request or per-message path - the gauges ride the existing 1 Hz pressure timer and add no work to any hot path. The counters record server decisions, not client behaviour - a client that disconnects mid-upgrade is counted in neither, so admitted + rejected can read below a load balancer's attempt count under flappy clients. Instrument failures are contained (a registry that throws on emit logs once and is silenced; one that throws at instrument creation fails at startup, loudly). No client identity (IP, session) ever appears in a label - the per-IP picture lives in the extensions per-IP bucket's own `upgrade_bucket_*` counters, and capability-cookie failures in its `capability_cookie_misses_total{reason}` (an app hook that rejects on a cookie miss surfaces here as `auth_rejected`).
...
- **Partial answers announce themselves.** Every document carries `metrics_snapshot_workers_expected` and `metrics_snapshot_workers_reporting`. When a worker misses the deadline they differ, and every summed series is understated for that scrape - alert on the difference rather than reading the dip as a real drop in traffic. A collection that fails outright sets `metrics_snapshot_degraded` to `1` instead: a worker that never heard back cannot know how many siblings it has, so the expected/reporting pair alone would read as complete. Alert on both.
...
- **A registry prefix does not break it.** `createMetrics({ prefix })` renames at registration, so a merge that recognised metrics by their rendered names would stop recognising any of them the moment you set one - and would go back to summing `open_fds` across workers. Recognition is by the adapter's own declared name, upstream of whatever your registry chooses to call it.
...
- **Cluster counters do not decrease.** Prometheus reads a decreasing counter as a reset and spikes `rate()`, so a counter that dips on a routine event prints a lie. A summed counter can dip for three distinct reasons, and each is handled separately: a worker that **exits** has its final counter totals carried forward, so a replacement starting at zero does not drop the sum; a **live** worker that misses the collection deadline contributes its last known counter totals rather than dropping out (a per-worker counter never decreases, so re-using its previous total undercounts it for one scrape instead of erasing it); and a **degraded** document omits counter families entirely rather than publishing one worker's fraction of them, because a gap reads as staleness while a smaller value reads as a reset. Only counters get this treatment - a gauge describes a live worker, and a stale connection count is a wrong number rather than a lagging one, so gauges do dip when a worker is missing. That is what `metrics_snapshot_workers_reporting` is for. The residual is one-directional: a total can lag reality by up to one collection interval of one worker's traffic, and does not go backwards.
...
There is deliberately no Grafana dashboard in the pack. Dashboard JSON is tied to a Grafana schema version and ages into a liability faster than anything else here, while `queries.md` gives you every expression needed to build one against whatever you actually run.
