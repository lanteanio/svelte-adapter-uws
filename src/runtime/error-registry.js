...
export const ADAPTER_ERROR_REGISTRY = Object.freeze([
	...
	Object.freeze({
		...
		automaticRecovery: 'None, and none should be expected from the next event: the fault is not in the record, so emitting a different one changes nothing until whatever altered the serialization is undone. If the very next diagnostic prints normally, what happened was momentary rather than a patched global.',
	...
	Object.freeze({
		...
		automaticRecovery: 'None for the lost notice. The sink is not unregistered and is called again for the next event, so a transient failure self-heals and a persistent one keeps printing this line beside each fallback-printed event.',
