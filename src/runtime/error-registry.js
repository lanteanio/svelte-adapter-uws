...
export const ADAPTER_ERROR_IDS = Object.freeze({
	...
	EGRESS_REFUSED: 'ADAPTER-ERR-EGRESS-REFUSED',
	EGRESS_TENANT_RESOLVER: 'ADAPTER-ERR-EGRESS-TENANT-RESOLVER',
	EGRESS_EVICTED: 'ADAPTER-ERR-EGRESS-EVICTED',
	...
	WS_SHUTDOWN_HOOK_UNSETTLED: 'ADAPTER-ERR-WS-SHUTDOWN-HOOK-UNSETTLED',
	...
	RESOURCE_GROWTH: 'ADAPTER-ERR-RESOURCE-GROWTH',
	RELAY_SPILL_QUARANTINE: 'ADAPTER-ERR-RELAY-SPILL-QUARANTINE',
	POSTURE_TRANSITION: 'ADAPTER-ERR-POSTURE-TRANSITION',
	WORKER_EXIT_SIGKILL: 'ADAPTER-ERR-WORKER-EXIT-SIGKILL'
});
...
export const ADAPTER_ERROR_REGISTRY = Object.freeze([
	Object.freeze({
		...
		automaticRecovery: 'None single-process or in acceptor mode. In reuseport mode the supervisor respawns the failed worker and the replacement retries the same bind - under a persistent conflict each attempt fails the same way until the slot exhausts its restart budget and the whole service goes down, the outcome ADAPTER-ERR-WORKER-RESTART-LIMIT documents from the other end.',
		nextAction: 'Check address availability, port conflicts, and bind permissions, then restart the process. In reuseport mode a repeating restart line for the same worker slot beside this one is the same conflict burning restart budget, not a second fault.',
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.EGRESS_REFUSED,
		code: null,
		event: 'egress.publish-refused',
		component: 'runtime.egress',
		severity: 'warn',
		emission: 'direct',
		problemPrefix: 'A publish crossed a configured egress ceiling and was refused.',
		messagePrefix: direct('runtime.egress', 'egress.publish-refused', 'warn', 'A publish crossed a configured egress ceiling and was refused.'),
		...
		automaticRecovery: 'Yes, by time: the window rotates (default 1000 ms) and publishing under the ceiling resumes on its own. Relayed frames from sibling workers are never refused.',
		nextAction: 'Decide whether the traffic or the ceiling is wrong. The attributes name the scope, the dimension (messages, bytes, or deliveries), and the configured limit; read the topic reference beside your `pressure.topPublishers` deliveries figures to see whether one publisher is spending the budget. Raise the ceiling in `websocket.egress` if the load is intended. On the dev plugin this event is the whole report: dev enforces the ceilings live but registers no metrics and reports its pressure egress figures as zeros, so the counts named above exist only in production and createTestServer.',
		sources: Object.freeze(['src/runtime/handler/egress-budget.js', 'src/vite.js', 'src/testing.js']),
		anchor: 'adapter-err-egress-refused',
		help: 'docs/errors.md#adapter-err-egress-refused'
	}),
	Object.freeze({
		id: ADAPTER_ERROR_IDS.EGRESS_EVICTED,
		code: null,
		event: 'egress.window-evicted',
		component: 'runtime.egress',
		severity: 'warn',
		emission: 'direct',
		problemPrefix: 'The egress ledger dropped a usage window that was still counting, so that key is unmetered for the rest of it.',
		messagePrefix: direct('runtime.egress', 'egress.window-evicted', 'warn', 'The egress ledger dropped a usage window that was still counting, so that key is unmetered for the rest of it.'),
		...
		automaticRecovery: 'Partly, and only by the traffic changing: the ledger holds its bound and keeps serving, and cardinality falling back under the bound restores full enforcement on its own. Nothing raises the bound.',
		nextAction: 'Treat it as a cardinality problem, not a capacity one. Check whether topic names embed unbounded identifiers, and scope tenant ids to the tenants you actually meter. The exact count is `egress_evicted_total{scope}` in production and createTestServer; the dev plugin registers no metrics, so there this line is the whole report.',
		sources: Object.freeze(['src/runtime/handler/egress-budget.js', 'src/vite.js', 'src/testing.js']),
		anchor: 'adapter-err-egress-evicted',
		help: 'docs/errors.md#adapter-err-egress-evicted'
	}),
	Object.freeze({
		id: ADAPTER_ERROR_IDS.EGRESS_TENANT_RESOLVER,
		code: null,
		event: 'egress.tenant-resolver-invalid',
		component: 'runtime.egress',
		severity: 'error',
		emission: 'direct',
		problemPrefix: 'The egress tenant resolver returned an unusable id; publishes are charged unattributed.',
		messagePrefix: direct('runtime.egress', 'egress.tenant-resolver-invalid', 'error', 'The egress tenant resolver returned an unusable id; publishes are charged unattributed.'),
		...
		automaticRecovery: 'None. The resolver stays installed and its valid answers keep working; only invalid results (and thrown calls) stay unattributed.',
		nextAction: 'Fix `egressTenantOf` to return a rule-conforming tenant id or null. The attributes carry the returned value TYPE only; reproduce locally by calling the resolver with the topics your server publishes.',
		sources: Object.freeze(['src/runtime/handler/egress-budget.js', 'src/vite.js', 'src/testing.js']),
		anchor: 'adapter-err-egress-tenant-resolver',
		help: 'docs/errors.md#adapter-err-egress-tenant-resolver'
	}),
	Object.freeze({
	...
	Object.freeze({
		...
		automaticRecovery: 'The next reload that succeeds applies the certificate and clears the degraded state.',
		nextAction: 'Confirm the served certificate matches the one on disk rather than assuming renewal succeeded, and read the TLS degraded state rather than the probe, which cannot see this. Treat the warning as expiry risk, not noise.',
	...
	Object.freeze({
		...
		automaticRecovery: 'A one-shot retry is armed from the failure itself, rather than from the next filesystem event, because the throw may have consumed the last event of a renewal burst and the next one could be months away. A persistent fault therefore retries at that cadence instead of spinning.',
		nextAction: 'Probe every SNI host with an HTTP request rather than only a handshake: both partial shapes complete handshakes while a host is either force-closing requests or still serving the boot certificate. Then read the attached error to pick the repair - certificate material explains only a throw inside the apply step, while a route-replay failure has nothing wrong with the material and resolves through the armed retry or a restart.',
		sources: Object.freeze(['src/runtime/handler/lifecycle.js']),
	...
	Object.freeze({
		...
		automaticRecovery: 'None for the watch itself: it is not retried, so this does not resolve without a restart. The arm-time catch-up may still swap in a renewal that was already on disk when the watch failed; the degraded state and its expiry sentinel survive even that success.',
	...
	Object.freeze({
		...
		sources: Object.freeze(['src/runtime/handler/lifecycle.js', 'src/vite.js', 'src/testing.js']),
		anchor: 'adapter-err-ws-shutdown-hook-threw',
		help: 'docs/errors.md#adapter-err-ws-shutdown-hook-threw'
	}),
	Object.freeze({
		id: ADAPTER_ERROR_IDS.WS_SHUTDOWN_HOOK_UNSETTLED,
		code: null,
		event: 'ws.shutdown-hook.unsettled',
		component: null,
		severity: 'error',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[ws] the WebSocket shutdown hook has not settled after ',
		...
		automaticRecovery: 'None by design: the budget exists so a wedged hook cannot hold the process open.',
		nextAction: 'Make the hook finish inside the budget, or raise SHUTDOWN_TIMEOUT. The hook receives a `signal` that aborts when the budget expires - honoring it turns this into a clean early return.',
		sources: Object.freeze(['src/runtime/handler/lifecycle.js', 'src/testing.js']),
		anchor: 'adapter-err-ws-shutdown-hook-unsettled',
		help: 'docs/errors.md#adapter-err-ws-shutdown-hook-unsettled'
	}),
	Object.freeze({
	...
	Object.freeze({
		...
		sources: Object.freeze(['src/runtime/utils/metrics.js', 'src/vite.js']),
	...
	Object.freeze({
		...
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[svelte-adapter-uws] the waiting-room renderer failed; serving the built-in English page instead',
	...
	Object.freeze({
		...
		sources: Object.freeze(['src/runtime/diagnostic.js', 'src/runtime/utils/operational-diagnostic.js']),
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.RESOURCE_GROWTH,
		code: null,
		event: 'ws.resource-growth.trending',
		component: null,
		severity: 'warn',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[ws] resource-growth auditor: ',
		...
		automaticRecovery: 'None. The auditor observes; it does not evict.',
		nextAction: "The line names the structure. Find the path that stopped shedding for it - a close, unsubscribe or eviction that no longer runs - rather than raising a limit; framework_resource_growth_suspected_total carries the structure as its `resource` label and shows whether the trend continued.",
		sources: Object.freeze(['src/runtime/handler.js']),
		anchor: 'adapter-err-resource-growth',
		help: 'docs/errors.md#adapter-err-resource-growth'
	}),
	Object.freeze({
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.POSTURE_TRANSITION,
		code: null,
		event: 'ws.protection-posture.transition',
		component: null,
		severity: 'warn',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[ws] protection posture ',
	...
	Object.freeze({
		...
		automaticRecovery: 'None inside the process. Recovery is the supervisor restarting it.',
		...
		sources: Object.freeze(['src/runtime/index.js']),
		anchor: 'adapter-err-worker-exit-sigkill',
		help: 'docs/errors.md#adapter-err-worker-exit-sigkill'
	})
]);
