...
export const ADAPTER_ERROR_IDS = Object.freeze({
	...
	DIAGNOSTIC_SINK_COLLAPSE: 'ADAPTER-ERR-DIAGNOSTIC-SINK-COLLAPSE',
...
export const ADAPTER_ERROR_REGISTRY = Object.freeze([
	...
	Object.freeze({
		...
		automaticRecovery: 'None for the gap itself. The subscribe still completes, on the ordinary no-coverage path rather than an error path.',
	...
	Object.freeze({
		...
		nextAction: 'Remove a stale socket file, fix the directory permissions, or point the export at a free path, then restart the worker.',
	...
	Object.freeze({
		...
		automaticRecovery: 'The export catches up on the next 1 Hz sample without waiting for another transition, and the next transition runs the handler again; a throw does not unregister it. Only the incident-timeline console line for this transition is gone for good.',
	...
	Object.freeze({
		...
		nextAction: 'Read the error printed with this line - it comes from the upgrade hook or the socket, and a repeated throw here means admission is letting through connections the upgrade cannot complete.',
	...
	Object.freeze({
		...
		automaticRecovery: 'None for the lost record. The next diagnostic is attempted normally, so a value-specific failure affects only events carrying that value.',
		nextAction: 'Read the event name on the line and find its emitter. Since stripping the attributes did not help, look at the envelope rather than the payload - a severity, event name or timestamp that is not the documented shape. If the emitter is application or plugin code, check its record against the documented diagnostic shape.',
		sources: Object.freeze(['src/runtime/diagnostic.js']),
		anchor: 'adapter-err-diagnostic-render-collapse',
		help: 'docs/errors.md#adapter-err-diagnostic-render-collapse'
	}),
	Object.freeze({
		id: ADAPTER_ERROR_IDS.DIAGNOSTIC_SINK_COLLAPSE,
		code: null,
		event: 'ws.diagnostic.sink-collapse',
		component: null,
		severity: 'error',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[ws] operational sink failed and the fallback could not render: ',
		...
		automaticRecovery: 'None. The sink is not unregistered and is called again for the next event, so a transient failure self-heals and a persistent one keeps printing this line.',
		nextAction: 'The sink is the thing to look at first - it is the component that failed originally, and it is supplied by the deployment rather than the adapter. The event name on the line says what was being reported when it went. If the sink is healthy in isolation, suspect what both paths share: a console replaced or removed by the host, which is the one dependency the fallback cannot route around.',
		sources: Object.freeze(['src/runtime/diagnostic.js']),
		anchor: 'adapter-err-diagnostic-sink-collapse',
		help: 'docs/errors.md#adapter-err-diagnostic-sink-collapse'
	}),
	Object.freeze({
	...
	Object.freeze({
		...
		automaticRecovery: "The worker exits and the primary replaces it. The dropped frames are not resent, so a client that was subscribed on that worker has a hole its own resume path must fill when it reconnects.",
