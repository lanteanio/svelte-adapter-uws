...
- **Consequence:** That diagnostic is lost, and because the cause is not carried by the record, so is every other one while it lasts - this is the operational stream going quiet, not one event dropping out of it. The event named on the line is whichever happened to be in flight first. The request path is unaffected: the failure is contained so telemetry cannot take a worker down with it.
- **Automatic recovery:** None, and none should be expected from the next event: the fault is not in the record, so emitting a different one changes nothing until whatever altered the serialization is undone. If the very next diagnostic prints normally, what happened was momentary rather than a patched global.
- **Next action:** Do not start at the emitter or the record - neither can produce this line. Look for what changed this process's JSON serialization: an APM or instrumentation agent, a polyfill, a test harness stubbing `JSON.stringify`, or a dependency that added `toJSON` to `Object.prototype`. Evaluating `JSON.stringify({})` in the same process settles it immediately. The event name on the line says only what was in flight when it went, and is not a suspect.
