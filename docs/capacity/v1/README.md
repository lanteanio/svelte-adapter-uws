...
5. one complete recovery window inside the declared recovery objective; and
...
| User SLO | `slo` | Peak p95/p99 and error ceiling plus recovery p95/error/window/deadline. Count timeouts and rejected application work as errors. |
...
`injectorDropped` means its own in-flight ceiling hid offered load. Scheduler
lag measures the generator's inability to reach intended start times. Either
condition fails generator integrity, so service SLO numbers from that run cannot
approve launch.
...
stable error/latency for the whole window.
