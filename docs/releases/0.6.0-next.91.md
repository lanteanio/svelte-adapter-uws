...
Static and prerendered dot-segment paths such as a stray `.env` or an unpacked `.git` now respond 404 by default instead of being served publicly, while `.well-known/*` discovery keeps working and an explicit option restores the previous behavior.
