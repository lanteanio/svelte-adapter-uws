...
- **The smooth command decode drops a duplicate id instead of applying it.**
  ...
  computed each id as the previous id plus a varint delta and accepted a zero
  delta, so a hand-built `0x03` ingress frame could deliver the same command
  id twice to the routed batch. Decode now drops any entry whose id fails to
  increase past the first - a zero delta, and the delta so large that float
  addition collapses onto the previous id, both spell a duplicate - and keeps
  the entries behind it intact
  by consuming the dropped entry's value before skipping it, so a crafted
  frame can neither double-apply a command nor shift the commands behind it.
...
- **The relay-gap emission derives from the error registry instead of
  ...
  with the `ADAPTER-ERR-RELAY-GAP` registry entry; they now come from the
  entry itself, so the registry and the wire cannot state different facts.
  The emitted values are unchanged, and the real-subscriber relay suite
  observes them as before. The desync coverage matrix now cites, for every
  delivery path, the test that holds its row as behaviour a real client
  observes - including new real-wire cases for cursor frame corruption with
  convergence, a mid-stream JSON fallback leaving the smooth delta chain
  intact, and explicit per-entry batch sequences arriving verbatim.
...
- **Erratum: appendix C.6's `RECORD_TOO_LARGE` row no longer turns the
  ...
  exceed a lowered limit and be conformingly refused. The same elision
  pattern existed next door: `SLOW_CONSUMER`'s gloss carried only
